...
import { DiscountValidation, ProductValidation } from '../Validation/Validation';
...
export class ProductController {
    ...
    public static async createProduct(req: Request<{}, {}, { productName: string, productPrice: number }>, res: Response) {
        try {
            ...
            return ResponseProcessor.endWithSuccess(res, {message: 'Product succesfully created.', status: 201, error: false, values: product});
        }
        catch(err: any) {
            return ResponseProcessor.endWithError(res, {message: err.message, error: true, status: 400})
        }
    }
    ...
    public static async changeProductProperty(req: Request<{ id: string }, {}, {productProperty: ProductProperty, newProductValue: string | number}>, res: Response) {
        try {
            const validationId = await ProductValidation.idSchema(req.params.id);
            ...
            return ResponseProcessor.endWithSuccess(res, {message: 'Product updated.', status: 200, error: false, values: updatedProduct});
        }
        catch(err: any) {
            return ResponseProcessor.endWithError(res, {message: err.message, error: true, status: 400})
        }
    ...
    public static async deleteProduct(req: Request<{ id: string}>, res: Response) {
        try {
            ...
            return ResponseProcessor.endWithSuccess(res, {message: 'Product deleted.', status: 204, error: false, values: deletedProduct});
        }
        catch(err: any) {
            return ResponseProcessor.endWithError(res, {message: err.message, error: true, status: 400})
        }
    ...
    public static async getAllProducts(req: Request, res: Response) {
        try {
            ...
            return ResponseProcessor.endWithSuccess(res, {message: 'Getting all products succesfull.', status: 200, error: false, values: allProducts});
        }
        catch(err: any) {
            return ResponseProcessor.endWithError(res, {message: err.message, error: true, status: 404})
        }
    }
    ...
    public static async addDiscountToProduct(req: Request< {id: string }, {}, { discountCode: string }>, res: Response) {
        try {
            ...
            return ResponseProcessor.endWithSuccess(res, {message: `Discount added to product with id: ${id}.`, status: 200, error: false, values: productWithDiscount});
        }
        catch(err: any) {
            return ResponseProcessor.endWithError(res, {message: err.message, error: true, status: 400})
        }
